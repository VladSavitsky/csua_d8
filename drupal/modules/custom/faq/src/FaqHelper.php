 ...
 * PHP version 5
 *
 * @category Drupal_Contib_Module
 * @package  FAQ
 * @author   Stella Power <stella.power@example.com>
 * @author   VladSavitsky <vlad.savitsky@gmail.com>
 * @license  http://www.gnu.org/copyleft/gpl.html  GNU General Public License 3
 * @link     https://www.drupal.org/project/faq
 */
...
class FaqHelper
{
     ...
     * @static
     * @access public
     * @return array Returns breadcrumbs.
     */
    public static function setFaqBreadcrumb($term = null)
    {
        ...
        if ($faq_settings->get('custom_breadcrumbs')) {
            ...
            $breadcrumb[] = l(
                t('Home'),
                null,
                array('attributes' => array('title' => $site_settings->get('name')))
            );
     ...
     *
     * @param int $tid Id of the tadonomy term to count nodes in.
     *
     * @return int Returns the count of the nodes in the given term.
     */
     ...
     *
     * @param object $term The category / term to display FAQs for.
     * @param string $theme_function Theme function to use to format the
     *     Q/A layout for sub-categories.
     * @param integer $default_weight Is 0 for $default_sorting = DESC; is 1000000 for
     *     $default_sorting = ASC.
     * @param string $default_sorting If 'DESC', nodes are sorted by creation date descending;
     *     if 'ASC', nodes are sorted by creation date ascending.
     * @param string $category_display The layout of categories which should be used.
     * @param string $class CSS class which the HTML div will be using. A special
     *     class name is required in order to hide and questions / answers.
     * @param object $parent_term The original, top-level,
     *     term we're displaying FAQs for.
     *
     * @return Returns rendered sub-categories.
     */
    public static function getChildCategoriesFaqs(
        $term,
        $theme_function,
        $default_weight,
        $default_sorting,
        ...
        foreach ($list as $tid => $child_term) {
            ...
            if (FaqHelper::taxonomyTermCountNodes($child_term->id())) {
                ...
                $ti_alias = $query->innerJoin(
                    'taxonomy_index',
                    'ti',
                    '(n.nid = %alias.nid)'
                );
                $w_alias = $query->leftJoin(
                    'faq_weights',
                    'w',
                    "%alias.tid = {$ti_alias}.tid AND n.nid = %alias.nid"
                );
                $query
                    ->fields('n', array('nid'))
                ...
                $query->addExpression(
                    "COALESCE(w.weight, $default_weight)",
                    'effective_weight'
                );
                // Doesn't work in Postgres.
                //$query->addExpression('COALESCE(w.weight,
                //   CAST(:default_weight as SIGNED))', 'effective_weight',
                //   array(':default_weight' => $default_weight));
                ...
                // We only want the first column, which is nid,
                // so that we can load all related nodes.
     ...
     *
     * @return An array of sub-categories.
     */
    public static function viewChildCategoryHeaders($term)
    {
        ...
        foreach ($list as $tid => $child_term) {
            ...
            if ($term_node_count) {
                ...
                //  $term_image = taxonomy_image_display($child_term->tid,
                //      array('class' => 'faq-tax-image'));
                //}

                $child_term_id = $child_term->id();
                $term_vars['link'] = l(
                    t($child_term->getName()),
                    "faq-page/$child_term_id"
                );
    ...
    public static function faqRelatedVocabularies()
    {
        ...
        foreach (\Drupal::entityManager()
            ...
            if ($field_definition->getType() == 'taxonomy_term_reference') {
                foreach ($field_definition->getSetting('allowed_values')
                    as $allowed_values
                ) {
    ...
    /**
     * Replacement for the old arg() function which is
     *     removed in drupal 8.0.0-alpha13
     * TODO: this should be replaced with the a path service when these
     *     are not changing any more.
     *
     * @param integer $id Number of the path's part.
     *
     * @return string The part of the path which indexed by the given id.
     */
